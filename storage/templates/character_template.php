...
function generateCharacterHTML($data)
{
    ...
    /* Period dates */
    $periodFrom = htmlspecialchars($data['period_from']  ?? $data['periodFrom'] ?? '');
    $periodTo   = htmlspecialchars($data['period_to']    ?? $data['periodTo']   ?? '');

    $fmtDate = function($d) {
        if (!$d) return '';
        $ts = strtotime($d);
        return $ts ? date('F Y', $ts) : $d;
    };
    $fromFormatted = $fmtDate($periodFrom);
    $toFormatted   = $fmtDate($periodTo);
    ...
    if (file_exists($sigHod) && function_exists('imagecreatefromstring')) {
        ...
        if ($src !== false) {
            $rot = (imagesy($src) > imagesx($src)) ? imagerotate($src, 270, 0) : $src;
            if ($rot !== false) {
    ...
    if ($fromFormatted && $toFormatted) {
        $periodSentence = "from <span class='b'>{$fromFormatted}</span> to <span class='b'>{$toFormatted}</span>";
    } elseif ($fromFormatted) {
        $periodSentence = "from <span class='b'>{$fromFormatted}</span>";
    }
...
<div class="content">
  <p>
    ...
    is / was a bonafide student of JD College of Engineering and Management,
    Katol Road, Nagpur.
  </p>
  <p>
    {$heShe} studied in
  ...
  <p>
    <span class="indent"></span>During {$hisHer} stay in this institution,
    {$hisHer} conduct and character were found to be
    <span class="bu">Good</span>.
    {$heShe} has never been involved in any disciplinary action or
    criminal offence.
  </p>
  <p>
    This certificate is issued on the basis of the records available
    with the institution for the purpose of
    <span class="bu">{$purpose}</span>.
  </p>
</div>
