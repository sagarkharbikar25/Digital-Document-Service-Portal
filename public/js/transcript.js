...
// ══════════════════════════════════════════════════════════════════
// 2️⃣ LOAD STUDENT DATA FROM LOCALSTORAGE
// ══════════════════════════════════════════════════════════════════
window.addEventListener('DOMContentLoaded', function () {
    ...
    // Specific Transcript.js Doc Type Change
...
document.getElementById('applicationForm').addEventListener('submit', async function (e) {
  ...
  try {
    ...
    const typeValue = docTypeEl.value;
    ...
    const response = await secureFetch(API_BASE + '/application/create', {
        ...
        headers:     { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            certificate_type: typeValue,
            purpose:          typeName + ' Application',
            branch:           document.getElementById('department').value,
            current_year:     document.getElementById('yearSem').value,
            academic_year:    '2024-25',
            extra_data:       Object.fromEntries(new FormData(this))
        })
    });
    ...
    const appNumber = res.application_number || res.application_id || 'N/A';
