/* ============================================================
   ...
   documents.js  |  Apply for Document page scripts
============================================================ */
...
function renderUser(u) {
    ...
    const branch = u.department || u.branch || '';
    ...
    for (let id in fields) {
        const el = document.getElementById(id);
        if (el && (!el.value || el.value === '--')) el.value = fields[id];
    }
}
  ...
  form.querySelectorAll('input[type="file"]').forEach(input => {
    Array.from(input.files).forEach(file => {
      if (file.size > 5 * 1024 * 1024) {
        alert(`File "${file.name}" exceeds 5 MB. Please upload a smaller file.`);
        isValid = false;
      }
    });
  });
...
document.getElementById('applicationForm').addEventListener('submit', async function (e) {
  ...
  try {
    ...
    // Construct detailed purpose context
    let purposeStr = '';
