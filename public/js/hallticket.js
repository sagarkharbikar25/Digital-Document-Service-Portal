...
function renderUser(u) {
    ...
    if (yearEl && !yearEl.value && u.current_year) yearEl.value = u.current_year;
}
...
function processFile(file, id, maxMB, types) {
    ...
    if (file.size > maxMB * 1024 * 1024) {
        if (errEl) { errEl.textContent = 'File too large.'; errEl.classList.add('show'); }
        return;
    }
    if (types && !types.includes(file.type)) {
        if (errEl) { errEl.textContent = 'Invalid file type.'; errEl.classList.add('show'); }
        return;
    }
...
async function submitForm() {
    ...
    try {
        ...
        document.getElementById('modalAppId').textContent = result.application_number || ('APP-' + Date.now());
